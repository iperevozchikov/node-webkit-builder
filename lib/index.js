...
NwBuilder.prototype.createReleaseFolder = function () {
    ...
    this._forEachPlatform(function (name, platform) {
        ...
        if(platform.platformSpecificManifest){
            fs.writeFileSync(path.join(platform.releasePath, 'package.json'), JSON.stringify(platform.platformSpecificManifest));
        }
            ...
            if(!self.options.macZip) {
                self._files.forEach(function (file) {
                    ...
                    if(file.dest === 'package.json' && platform.platformSpecificManifest){
                        copiedFiles.push(Utils.copyFile(path.join(platform.releasePath, 'package.json'), dest));
                    }
                    else {
