...
NwBuilder.prototype.preparePlatformSpecificManifests = function(){
    ...
    this._forEachPlatform(function (name, platform) {
        promises.push(new Promise(function(resolve, reject) {
            ...
            var platformOverrides = self._appPkg.platformOverrides[name];
            if (platformOverrides && Object.keys(platformOverrides).length) {

                platform.platformSpecificManifest = _.merge(
                    _.clone(self._appPkg, true),
                    platformOverrides,
                    function(pkgValue, overrideValue){
                        /*
                         * if overrides.x is {} but sourceManifest.x is a non-empty object {prop:0, another:2},
                         * take the {}
                         */
                        if(_.isPlainObject(pkgValue) && _.isPlainObject(overrideValue) && _.isEmpty(overrideValue)){
                            return overrideValue
                        }
                    }
                );
                delete platform.platformSpecificManifest.platformOverrides;
                resolve();
            }
            else {
                resolve();
...
NwBuilder.prototype.zipAppFiles = function () {
    ...
    var _needsZip = false,
        numberOfPlatformAgnosticZips = 0;
    ...
    this._forEachPlatform(function(name, platform) {
        if(name === 'osx' && self.options.macZip || platform.needsZip) {
            ...
            self.zips[name] = { platformSpecific: platformSpecific };

            numberOfPlatformAgnosticZips += !platformSpecific;
        }
    });
    ...
    return new Promise(function(resolve, reject) {
        ...
        new Promise(function(resolve, reject) {
            if(numberOfPlatformAgnosticZips > 1){
        ...
        .then(function(platformAgnosticZip){
            ...
            self.zips.forEach(function(zip, platformName){
