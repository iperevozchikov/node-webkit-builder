...
NwBuilder.prototype.build = function (callback) {
    ...
    this.checkFiles().bind(this)
        ...
        .then(this.platformSpecificManifests)
...
NwBuilder.prototype.platformSpecificManifests = function(){
    if(Object.keys(this.platformOverrides).length === 0){
        return true;
    }
    ...
    this._platforms.forEach(function (single_platform) {
        promises.push(new Promise(function(resolve, reject) {
            ...
            var platformOverrides = self.platformOverrides[single_platform.platform];
            if (platformOverrides && Object.keys(platformOverrides).length) {
                var platformSpecificOptions = _.defaults(platformOverrides, options);
                resolve();
            }
            else {
                resolve();
